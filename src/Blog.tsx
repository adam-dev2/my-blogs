...
import { POSTS } from "./Blogs/Blogs.ts";
...
function ScrollFade({
  ...
  return (
    <div className={`relative overflow-hidden ${className ?? ""}`}>
      ...
      <div
        className={`pointer-events-none absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-[#141417] to-transparent transition-opacity ${
          atBottom ? "opacity-0" : "opacity-100"
        }`}
      />
      <div
        className={`pointer-events-none absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-[#141417] to-transparent transition-opacity ${
    ...
    <div className="min-h-screen text-white font-sans px-6 pt-24  bg-[#0a0a0c] bg-grid-pattern">
      {!selected ? (
        <div className="w-full max-w-2xl mx-auto border border-[#26262b] rounded-xl bg-[#141417] p-6">
          ...
          <div className="border border-[#26262b] rounded-lg overflow-hidden bg-[#0e0e10]">
            {POSTS.map((post, i) => (
              <EntryRow
                key={post.id}
                post={post}
